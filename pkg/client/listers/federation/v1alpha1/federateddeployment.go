...
import (
	v1alpha1 "github.com/font/fedv2-crd-validation/pkg/apis/federation/v1alpha1"
...
// List lists all FederatedDeployments in the indexer.
func (s *federatedDeploymentLister) List(selector labels.Selector) (ret []*v1alpha1.FederatedDeployment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.FederatedDeployment))
	})
	return ret, err
}
...
// List lists all FederatedDeployments in the indexer for a given namespace.
func (s federatedDeploymentNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.FederatedDeployment, err error) {
...
// Get retrieves the FederatedDeployment from the indexer for a given namespace and name.
func (s federatedDeploymentNamespaceLister) Get(name string) (*v1alpha1.FederatedDeployment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("federateddeployment"), name)
	}
	return obj.(*v1alpha1.FederatedDeployment), nil
}
